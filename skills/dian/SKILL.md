...
A protocol for staying focused within a session. Dian does not touch git boundaries (pull/push). That's seach's job. Dian keeps you on track once you're working.
...
**State file:** When entering a phase, write the current phase to `kivna/.active-modes`. When closing out, remove the dian line from the file (or delete the file if it's the only entry). This lets `/kerd:seach in` report active modes.
...
- **No git boundary ops.** No `git pull`, no `git push`. Use `/kerd:seach` for that.
